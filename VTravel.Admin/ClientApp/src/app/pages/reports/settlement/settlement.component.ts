...
export class SettlementComponent implements OnInit {
  ...
  loadBookingWiseHtml() {
    let headers = new HttpHeaders().set("Authorization", "Bearer " +
      this.token).set("Content-Type", "application/json");
    this.http.get('api/reports/get-bookingwise-html?propid=' + this.searchdata.property
      + '&fromdate=' + this.formatDate(this.searchdata.fromDate)
      + '&todate=' + this.formatDate(this.searchdata.toDate)
      , { headers: headers }).subscribe((res: any) => {
        if (res.actionStatus == 'SUCCESS') {
          this.bookinghtml = res.data;
        }
        else {
        ...
        error => {
          ...
          console.log('api/reports/get-bookingwise-html', error)
          if (error.status === 401) {
            this.router.navigate(['auth/login']);
          }
        });
  ...
  exportChannelWiseToExcel() {
    ...
    // Encode the Excel template in base64 and set it as the href attribute
