        ...
        public IActionResult GetPlanDetails(int id)
        {
            ...
            try
            {
                ...
                var query = string.Format(@"(select distinct IFNULL(rb.id, 0) id, r.id room_id, r.title room, m.id mealid, m.mealplan, o.id occid, o.occupancy, IFNULL(rb.rate, 0) rate
                                            ...
                                            inner join mealplans m on m.id = rm.mealplan
                                            left join rateplan_breakup rb on rb.rateplan = rp.id and rb.room_id = r.id and o.id = rb.occupancy and m.id = rb.mealplan
                                            where rp.id = {0} order by r.id) order by room_id, mealid, occid;"
                                   , id);
        ...
        public IActionResult GetAssignedplanList(int id, string from, string to)
        {
            ...
            try
            {
                ...
                var query = string.Format(@"select i.id, i.property_id propertyId, DATE_FORMAT(i.inv_date, '%Y-%m-%d') invDate, IFNULL(i.rateplan, 0) rp_id, IFNULL(rp.name, '') rp_name
                                            , IFNULL(rp.color, '#ffffff') rp_color, MAX(IFNULL(i.updated_on, i.created_on)) updated_on 
                                            from property p
											...
											group by i.property_id, i.inv_date having updated_on = MAX(IFNULL(i.updated_on, i.created_on));"
                                   , id, from, to);
                ...
                //}
                //connectionString = VTravel.UAEAdmin.Startup.conStr;
